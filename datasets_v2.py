...
class ImageDataset_paper(Dataset):
    def __init__(self, root, mode="train", use_mask=False):
        ...
        self.train_input_files = sorted(glob.glob(os.path.join(root, "train_input" + "/*.jpg")))
        self.train_target_files = sorted(
            glob.glob(os.path.join(root, "train_target" + "/*.jpg")))
        # self.train_mask_files = sorted(glob.glob(os.path.join(root, "autodl-tmp/masks_360p" + "/*.png")))

        self.test_input_files = sorted(glob.glob(os.path.join(root, "test_input" + "/*.jpg")))
        self.test_target_files = sorted(
            glob.glob(os.path.join(root, "test_target" + "/*.jpg")))
